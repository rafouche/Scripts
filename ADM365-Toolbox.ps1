...
function Find-ImmutableIdOwner {
    ...
    try {
        ...
        Write-HMLog "  Deleted-items conflict search error: $_" 'WARN'
    }
...
function Show-ConflictDialog {
    ...
    if ($ConflictUser) {
        ...
        if ($isDeleted) {
            $lDel = New-Lbl 'SOFT-DELETED (in recycle bin) - clearing this requires a permanent purge, not a property edit.' -Color $script:HMColors.Warning
            $lDel.Location = [System.Drawing.Point]::new(8,80); $pCard.Controls.Add($lDel)
        }
    } else {
        $lNF = New-Lbl 'Conflicting account could not be looked up (may be a contact, group, or device object).' -Color $script:HMColors.Warning
    ...
    $btnClear.Add_Click({
        if ($ConflictUser -and $isDeleted) {
            ...
            try {
                Write-HMLog "  Permanently deleting soft-deleted conflicting object: $($ConflictUser.userPrincipalName)" 'WARN'
                Invoke-MgGraphRequest -Method DELETE `
                    -Uri "https://graph.microsoft.com/v1.0/directory/deletedItems/$($ConflictUser.id)" -EA Stop
                Write-HMLog "  Conflict purged: $($ConflictUser.userPrincipalName)." 'OK'
                $script:_conflictResult = 'Cleared'
            } catch {
                Write-HMLog "  Failed to purge conflict: $_" 'ERR'
                [System.Windows.Forms.MessageBox]::Show(
